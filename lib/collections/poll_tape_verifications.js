...
PollTapeVerifications.attachSchema(new SimpleSchema({
  // Tries to estimate when the user started the verification, before its
  // creation time. Look for `pollTapeVerificationStartedAt` for use.
  ptvStartedAt: {
    type: Date,
    optional: true
  },
  ...
  areCityAndStateCorrect: _({
    type: Boolean,
    label: "5. Cidade e estado conferem?"
  }).extend(requiredIfValidSubmission),
  electionRound: _({
    type: Number,
    label: "6. Turno (1 ou 2):",
    min: 1,
    max: 2
  }).extend(requiredIfValidSubmission),
  electionZone: _({
    type: Number,
    label: "7. Zona eleitoral:",
    min: 0,
    max: 9999
  }).extend(requiredIfValidSubmission),
  electoralSection: _({
    type: Number,
    label: "8. Seção eleitoral:",
    min: 0,
    max: 9999
  }).extend(requiredIfValidSubmission),
  isHonorCoded: _({
...
if (Meteor.isServer) {
  PollTapeVerifications._ensureIndex({ userId: 1, pollTapeSubmissionId: 1 },
    { unique: true });
}
